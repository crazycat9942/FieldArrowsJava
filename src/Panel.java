...
public class Panel extends JPanel {
    ...
    Graphics g;
    double time = 0;
    double window_screenX, window_screenY;
    Menu menu = new Menu(this);
    double zoomFactor;
    double maxCoordX, minCoordX;
    double maxMagnitude, maxCurl, maxDivergence;
    double maxCoordY, minCoordY;
    double[] origMaxMin;
    Point userMouse;
    Axes axes;
    int arrowsX = 40, arrowsY = 40;
    JEP parser;
    ...
    Panel(double tempSX, double tempSY)
    {
        window_screenX = tempSX;
        window_screenY = tempSY;
        zoomFactor = 10;
        maxCoordX = zoomFactor;
        minCoordX = -maxCoordX;
        maxMagnitude = maxCurl = maxDivergence = -1;
        maxCoordY = maxCoordX *window_screenY/window_screenX;
        minCoordY = -maxCoordY;
        origMaxMin = new double[]{maxCoordX, maxCoordY, minCoordX, minCoordY};
        axes = new Axes(this);
        parser = new JEP();
        ...
        this.addMouseWheelListener(new MouseWheelListener() {
            ...
            public void mouseWheelMoved(MouseWheelEvent e) {
                ...
                double[] tempPoint = inverseZoom.preMultiply(new double[]{window_screenX, window_screenY, 1});
    ...
    public void colorComplex(){
        ...
        else
        {
            ...
            else
            {
                ...
                colors = new int[(int)window_screenX][(int)window_screenY];
                Complex m;
                double argument;
                double modulus;
                Complex R;
        ...
        {
            colors = new int[(int) window_screenX][(int) window_screenY];
            if(true || zoomFactor >= 1000000000000.) {
                ArrayList<Complex2> XTemp = new ArrayList<>();//computed at high precision
                Point2D.Double centerPoint = new Point2D.Double(screenToCoordsX(window_screenX/2), screenToCoordsY(window_screenY/2));
                XTemp.add(new Complex2(new BigDecimal(centerPoint.x).setScale(50, RoundingMode.FLOOR), new BigDecimal(centerPoint.y).setScale(50, RoundingMode.FLOOR)));
                Complex2 x0 = XTemp.getFirst();
                for (int n = 1; n < 500; n++) {
                    XTemp.add(XTemp.get(n - 1).mul(XTemp.get(n - 1)).add(x0));
                    double temp = XTemp.getLast().magnitude().doubleValue();
                    if(temp >= 2 || Double.isNaN(temp) || Double.isInfinite(temp))
                    {
                        break;
                    }
                }
                ArrayList<Complex> X = new ArrayList<>();//low precision
                for (Complex2 a: XTemp)
                {
                    X.add(a.toComplex());
                }
                ...
                for (int n = 1; n < X.size(); n++) {
                    A.add(add(A.get(n - 1).mul(X.get(n - 1)).mul(2), new Complex(1, 0)));//2X_n*A_n + 1
                    B.add(add(X.get(n - 1).mul(2).mul(B.get(n - 1)), A.get(n - 1).mul(A.get(n - 1))));//2X_n*B_n + A_n^2
                    C.add(add(X.get(n - 1).mul(2).mul(C.get(n - 1)), (A.get(n - 1).mul(2).mul(B.get(n - 1)))));//2X_n*C_n + 2A_n*B_n
                }
                //g.setColor(mandelbrotColor((int)window_screenX/2, (int)window_screenY/2));
                //g.fillRect((int)window_screenX/2, (int)window_screenY/2, 1, 1);
                Complex X0 = x0.toComplex();
                for (int i = (int)window_screenX/2 - 100; i < (int)window_screenX/2 + 100; i++) {
                    for (int j = (int)window_screenY/2 - 100; j < (int)window_screenY/2 + 100; j++) {
                        if(false && i > (int) window_screenX / 2.5 && j > (int) window_screenY / 2.5 && i < (int)window_screenX/2 && j < (int)window_screenY/2)
                        {
                            colors[i][j] = mandelbrotColor(new Complex(screenToCoordsX(i), screenToCoordsY(j))).getRGB();
                        }
                        else
                        {
                            Complex delta0 = add(new Complex(screenToCoordsX(i), screenToCoordsY(j)), X0.mul(-1));
                            Complex deltasq = delta0.mul(delta0);
                            Complex Y_n = add(delta0, X.getFirst());//Y_0
                            Complex delta_n = delta0;//delta
                            for (int k = 1; k < X.size() + 1; k++) {
                                if (k == X.size())
                                {
                                    colors[i][j] = Color.black.getRGB();
                                    break;
                                }
                            /*temp3 = add(temp3.mul(temp3), temp2);
                            double temp4 = temp3.abs();
                            if(temp4 >= 2 || Double.isNaN(temp4) || Double.isInfinite(temp4))
                            {
                                double h = (Math.log(k + 15) * 100) % 360;
                                colors[i][j] = HCLToRGB(h, 100, 2).getRGB();
                                break;
                            }*/
                                if(zoomFactor < 100000) {
                                    delta_n = add(X.get(k - 1).mul(2).mul(delta_n), delta_n.mul(delta_n), delta0);//delta_k//Y_k
                                }
                                else {
                                    delta_n = add(A.get(k-1).mul(delta0), B.get(k).mul(deltasq), C.get(k).mul(delta0).mul(deltasq));
                                }
                                Y_n = add(delta_n, X.get(k-1));
                                double mag = Y_n.abs();//delta0.mul(add(A.get(k), B.get(k).mul(delta0), C.get(k).mul(deltasq), X.get(k))).abs();
                                if (mag >= 2) {
                                    double h = (Math.log(k + 15) * 100) % 360;
                                    colors[i][j] = HCLToRGB(h, 100, 2).getRGB();
                                    break;
            ...
            if(false) {
                if (kernel1 == null) {
                    ...
                    range = Range.create(8);
                }
            }
            //tempImage = createImage(colors);
            if (false && !zoomChanged && !panChanged && !keyChanged && tempImage != null){// && zoomFactor > 1000000000000.) {
                g.drawImage(tempImage, 0, 0, null);
            }
            /*else if(panChanged && tempImage != null)
            {
                ...
                if(false && zoomFactor < 1000000000000.) {
                    kernel1.execute(range);//remove this comment
                    tempImage = createImage(colors);
    ...
    {
        Color tempColor = mandelbrotColor(minI, minJ);
        if(!mandelbrotColor(maxI, maxJ).equals(tempColor) || !mandelbrotColor(maxI, minJ).equals(tempColor) || !mandelbrotColor(minI, maxJ).equals(tempColor))
        {
            return false;
        }
    ...
    public Color mandelbrotColor(Complex z)
    {
        ...
        double p = Math.sqrt(x0*x0 - 0.5*x0 + 0.0625 + y0*y0);//cardioid/bulb checking
        if(x0 <= p - 2*p*p + 0.25 || x0*x0 + 2*x0 + 1 + y0*y0 <= 0.0625)
        {
            i = maxIter;
        }
        while(i < maxIter && x2 + y2 < 4)// && System.nanoTime() - start < 100000)
        {
            /*zy *= zx;
            zy += zy + cy;
            zx = xsqr - ysqr + cx;
    ...
    protected void paintComponent(Graphics gInit) {
        ...
        window_screenX = this.getWidth();
        window_screenY = this.getHeight();
        axes.tickDistance *= (axes.SX/window_screenX + axes.SY/window_screenY)/2;
        axes.SX = window_screenX;
        axes.SY = window_screenY;

        if(arrows.getFirst().maxSX != window_screenX || arrows.getFirst().maxSY != window_screenY) {//updates screen position of arrows if window size changed
            for (Arrow i : arrows)
            {
                if (i.maxSX != window_screenX) {
                    i.screenX *= window_screenX / i.maxSX;
                    i.maxSX = window_screenX;
                }
                if (i.maxSY != window_screenY) {
                    i.screenY *= window_screenY / i.maxSY;
                    i.maxSY = window_screenY;
                }
            }
        }
    ...
    public Complex add(Complex c1, Complex c2, Complex c3)
    {
        return new Complex(c1.re() + c2.re() + c3.re(), c1.im() + c2.im() + c3.im());
    }
    public Complex add(Complex c1, Complex c2, Complex c3, Complex c4)
    {
        return new Complex(c1.re() + c2.re() + c3.re() + c4.re(), c1.im() + c2.im() + c3.im() + c4.im());
    }
}
